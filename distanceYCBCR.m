...
load('OCLBP_YCBCR_FEATURE.mat');
image_no=1:400;
classes=[];
sz=size(oc_lbp_YCBCR);
no_img=sz(1);
FV=oc_lbp_YCBCR;
...
for i= 1:no_img
    ...
    for j=1:no_img 
        ...
        % imga=imread(I);
         %J=strcat('image (', num2str(l), ').jpg');
         %ycbcr=YCBCR2ycbcr(imga);
        % newimage=imread(J);
         
         %v=ycbcr(k,)-ycbcr(l,)
        v=FV(i,:)-FV(j,:);
        distance_YCBCR(i,j)=sqrt( sum(v.^2));
    end
i    
end
...
for i=1:400
    [B(i,:),I(i,:)] = sort(distance_YCBCR(i,:),'Ascend');
end
...
for im=1:400
    ...
    %fscore(im,1)=2.*((prec1.*recall1)/(prec1+recall1)).*100;
...
result_YCBCR_400=[avg_pre avg_rec avg_fscore]
save('YCBCR_400','avg_pre','avg_rec','avg_fscore','distance_YCBCR')
